...
function getNovelTitleFromMeta(meta) {
    if (meta === null || meta === void 0 ? void 0 : meta.novel) {
        let arr = [
            ...
            'title_tw',
            'title_cn',
        ].concat((0, index_1.filterByPrefixReturnKeys)('title_', meta.novel))
    ...
    function sortSubKey(key, sortList, unique) {
        ...
        if (Array.isArray(obj)) {
            ...
            if (unique) {
                ...
                parent[key] = (0, array_hyper_unique_1.array_unique)(parent[key]);
...
function chkInfo(ret, options = {}) {
    ...
    if (ret.novel) {
        [
            ...
            if (k in ret.novel) {
                ret.novel[k] = (0, index_1.anyToArray)(ret.novel[k], true);
            }
        });
        if ('novel_status' in ret.novel) {
            ret.novel.novel_status = (0, env_bool_1.envVal)(ret.novel.novel_status);
    ...
    if ('contribute' in ret) {
        ret.contribute = (0, index_1.anyToArray)(ret.contribute, true);
    }
    ...
    if (ret.options) {
        if (typeof ret.options.textlayout === 'object') {
            Object.entries(ret.options.textlayout)
                .forEach(([k, v]) => ret.options.textlayout[k] = (0, env_bool_1.envVal)(v));
        }
        if (typeof ret.options.downloadOptions === 'object') {
            Object.entries(ret.options.downloadOptions)
                .forEach(([k, v]) => ret.options.downloadOptions[k] = (0, env_bool_1.envVal)(v));
        }
    }
    return ret;
}
